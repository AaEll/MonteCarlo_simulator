    ...
    while n<no_episodes:
        ...
        while True:
            try:
                ...
                _w_ = _w_+ alpha*(r_ + MDP.damping_constant*np.dot(basis(s_),_w_[:,a_])-np.matmul(basis(s),_w_[:,a_]))*delta_q(basis(s),a)
                a = a_
                s = s_
    ...
    while n<no_episodes:
        ...
        while True:
            try:
                ...
                _w_ = _w_+ alpha*(r_ + MDP.damping_constant*np.dot(basis(s_),_w_[:,a_])-np.max(np.matmul(basis(s),_w_)))*delta_q(basis(s),a)
                current_reward += r_*(MDP.damping_constant**t)                
                a = a_
                s = s_
                t+=1
